...
namespace MyDictionary
{
     ...
     * 我们假设通过 hash 计算,这个键值对元素应该放在 bucket[0] 这个桶里面,因此这时候 bucket[0] = 0 ,等号右边的 0 表示 Entry[] 的下标.
     * 我们再 Add(4,value),这时候,Entry[] 有2个元素,Entry[0] 和 Entry[1],
     * 并且我们假设通过 hash 计算,这个键值对元素也应该放在 bucket[0] 这个桶里面,
     * 那么,这时候 bucket[0] 就不在 = 0了,而是 = 1,
     * 并且 Entry[1] 中的元素(Entry 类型)有个 next 字段,其值 = 0,  这个 0 就是 Entry[] 的索引,表示该元素的下个元素是 Entry[0] ,进一步说就是
     * 桶0里面有两个元素,最上面的(第2次放进去的)是 Entry[1] ,第2个(第1次放进去的那个)是 Entry[0],并且通过 Entry[1] 是可以找到 Entry[0] 的.
    ...
    public class MyDictionary<TKey, TValue>
    {
        ...
        private int count;
        private Entry[] entries; //实际存放元素的数组.
        ...
        {
            var prime = MyHashHelpers.GetPrime(capacity);
            buckets = new int[prime];
            for (var index = 0; index < buckets.Length; ++index) buckets[index] = -1;
        ...
        private void Resize(int newSize, bool forceNewHashCodes)
        {
            ...
            for (var index = 0; index < numArray.Length; ++index) numArray[index] = -1;
